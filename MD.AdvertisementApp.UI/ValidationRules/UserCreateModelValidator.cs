...
namespace MD.AdvertisementApp.UI.ValidationRules
{
    public class UserCreateModelValidator:AbstractValidator<UserCreateModel>
    {
        public UserCreateModelValidator()
        {
            ...
            RuleFor(x => x.Password).MinimumLength(8).WithMessage("Şifrə minumum 8 simvol ola bilər"); ;
            RuleFor(x => x.Password).Equal(x => x.ConfirmPassword).WithMessage("Təkrar şifrə uyğun deyil !!!");
            RuleFor(x => x.UserName).MinimumLength(5).WithMessage("İstifadəçi adı minumum 5 simvol ola bilər"); ;
            ...
            RuleFor(x => new{x.UserName,x.FirstName}).Must(x => NotContain(x.UserName, x.FirstName)).WithMessage("Istifadəçi adı ilə adınız eyni ola bilməz !!!").When(x=>x.UserName!=null && x.FirstName!=null); 
        }
